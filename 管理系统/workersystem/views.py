...
def addadmin(request):
    ...
    if(sex=='男'):
        ...
        sex=1
    conn = pymysql.connect(host='localhost', user='root', password='', database='a05', port=3306)
    cursor = conn.cursor()
...
def workersignup(request):
   # try:
    ...
    conn = pymysql.connect(host='localhost', user='root', password='root', database='a05', port=3306)
    cursor = conn.cursor()
    if(gender=='男'):
        gender=0
    else:
...
def getdata2(request):
    ...
    # 查询整个Master
    # print(master_list.decode('utf-8'))
    ret = Workers.objects.all()
    # print(ret.get(workerid="021"))
    lenth=ret.count()
    print(lenth)
    json_list = []
    k=1
    for i in range(lenth):
        ...
        if (k >= (1+(int(page)-1)*int(pageSize)) and k<=int(pageSize)*int(page)):
            # worker=Workers.objects.get(id=i)
            # print(worker)
