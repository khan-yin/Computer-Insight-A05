...
class MtcnnDetector(object):
    ...
    @staticmethod
    def box_expand(boxes_align, zoom_ratio, up_t_ratio):
        x1y1x2y2 = boxes_align[:,:4]
        w = x1y1x2y2[:,2]-x1y1x2y2[:,0]
        h = x1y1x2y2[:,3]-x1y1x2y2[:,1]
        print(w)
        wh_offset = np.stack((-w, -h, w, h),axis=1)*((zoom_ratio)/2)
        wh_offset[:,1] -= h * up_t_ratio
        wh_offset[:,3] -= h * up_t_ratio
        print(wh_offset)
        x1y1x2y2 += wh_offset
        np.clip(x1y1x2y2, a_min=0, a_max=1)
        return x1y1x2y2
